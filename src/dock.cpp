...
QScrollArea#LowerThirdContent QPushButton:hover { background: rgba(255,255,255,0.06); border-radius: 3px; }
...

	{
		...
		updateLabel_ = new QLabel(tr("New MediaScope Lower Thirds update available."), updateFrame_);
		updateLabel_->setObjectName(QStringLiteral("sltUpdateBannerLabel"));
		updateLabel_->setWordWrap(true);
		row->addWidget(updateLabel_, 1);

		// Update banner intentionally has no outbound link or remote check --
		// this build is self-contained. Kept hidden/local-only.
		updateBtn_ = nullptr;
...

	{
		...
		browserSourceCombo->setToolTip(
			tr("Select an existing OBS Browser Source that will display and control MediaScope Lower Thirds"));
...

	{
		...
		addBtn->setToolTip(tr("Add new lower third (choose a style)"));
		addBtn->setFlat(true);
...
void LowerThirdDock::onAddLowerThird()
{
	...
	QMenu menu(this);
	menu.setTitle(tr("Choose a style"));

	const int count = vflow::style_template_count();
	for (int i = 0; i < count; ++i) {
		const QString name = QString::fromStdString(vflow::style_template_name(i));
		QAction *act = menu.addAction(name);
		connect(act, &QAction::triggered, this, [this, i]() {
			vflow::add_lower_third_with_style(i);
			updateRowCountdowns();
			emit requestSave();
		});
	}
