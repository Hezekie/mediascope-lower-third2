...
LowerThirdSettingsDialog::LowerThirdSettingsDialog(QWidget *parent) : QDialog(parent)
{
	...
	}
	{
		...
		help->setText(tr(
			...
			"When enabled, MediaScope Lower Thirds creates a per-lower-third JSON file named:\n"
			"  parameters_<ID>.json\n\n"
...

	{
		...
		// Marketplace/browse-online features have been removed for this build --
		// no outbound requests or link-outs are made. The list stays local-only.
		seeAllLowerThirdsBtn->setVisible(false);
...
void LowerThirdSettingsDialog::openTemplateEditorDialog(const QString &title, QPlainTextEdit *sourceEdit)
{
	...
	QSettings s(QSettings::IniFormat, QSettings::UserScope, QStringLiteral("MMLTech"),
		    QStringLiteral("mediascope-lower-thirds"));
